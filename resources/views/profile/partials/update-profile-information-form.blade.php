<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="etablissement" :value="__('Nom de l\'établissement')" />
            <x-text-input id="etablissement" name="etablissement" type="text" class="mt-1 block w-full" 
                :value="old('etablissement', $user->etablissement)" autocomplete="organization" />
            <x-input-error class="mt-2" :messages="$errors->get('etablissement')" />
        </div>       
